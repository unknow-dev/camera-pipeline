...
int main(int argc, char **argv) {
  ...
  // 				      in_bounded(x, y+3, c)));
  // h_sharp_y(x, y, c) = (h_sharp_k(0) * in_bounded(x, y) +
  // 		     h_sharp_k(1) * (in_bounded(x, y-1) +
  // 				     in_bounded(x, y+1)) +
  // 		     h_sharp_k(2) * (in_bounded(x, y-2) +
  // 				     in_bounded(x, y+2)) +
  // 		      h_sharp_k(3) * (in_bounded(x, y-3) +
  // 				     in_bounded(x, y+3)));
  // Func h_sharp;
  // h_sharp(x, y, c) = (h_sharp_k(0) * h_sharp_y(x, y) +
  // 		      h_sharp_k(1) * (h_sharp_y(x-1, y) +
  // 				      h_sharp_y(x+1, y)) +
  // 		      h_sharp_k(2) * (h_sharp_y(x-2, y) +
  // 				      h_sharp_y(x+2, y)) +
  // 		      h_sharp_k(3) * (h_sharp_y(x-2, y) +
  // 				      h_sharp_y(x+2, y)));
  ...
  // Func h_sharp_u8;
  // h_sharp_u8(x, y) = u8(h_sharp(x, y));

  // Buffer<uint8_t> output = h_sharp_u8.realize(in.width(), in.height()// , in.channels()
  // 					      ); 
  // save_image(output, "../images/outputs/h_sharp.jpg");
