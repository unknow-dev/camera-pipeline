...
int main(int argc, char **argv) {
  ...
  for (int count = 0; count <= steps; count++) {
    ...
    bool should_print = (count == 0 || count == steps/2 || count == steps);
    if(should_print) {
      printf("Iteration %d\n"
  	     "Coefficients: ", count);
      for (int j = 0; j < kernel_size; j++) {
    ...
    p.compile_to_static_library("batch", {batch, h_inv_ca_rg_k_b}, "batch");
  ...
  r_i__.compile_to_static_library(")

  // save_image(b_, "../images/batch_test/output/"+to_string(0)+".jpg");

  for (int count = 0; count < batch_size; count++) {
    Buffer<uint8_t> b_ = r_i__b.sliced(2, count);
    save_image(b_, "/home/zendevil/Desktop/Halide/images/batch_test/output/"+to_string(count)+".png");
  }
